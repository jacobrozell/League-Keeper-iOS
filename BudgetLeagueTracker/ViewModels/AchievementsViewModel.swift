...
final class AchievementsViewModel {
    ...
    /// Whether the template picker sheet is showing.
    var isShowingTemplatePicker: Bool = false
    ...
    var balanceSummary: AchievementBalanceSummary {
        let alwaysOnCount = achievements.filter(\.alwaysOn).count
        let randomPerWeek = AppConstants.League.defaultRandomAchievementsPerWeek
        let expectedActive = alwaysOnCount + min(randomPerWeek, max(achievements.count - alwaysOnCount, 0))
        let averagePoints = achievements.isEmpty
            ? 0
            : Double(achievements.map(\.points).reduce(0, +)) / Double(achievements.count)

        let guidance: String?
        if achievements.count < 3 {
            guidance = "Add a few more for variety each week."
        } else if alwaysOnCount > 6 {
            guidance = "Many every-week achievements — the random pool may feel redundant."
        } else if averagePoints * Double(expectedActive) > 12 {
            guidance = "Achievement points may dominate placement — consider lowering values or reducing random count."
        } else {
            guidance = nil
        }

        return AchievementBalanceSummary(
            catalogCount: achievements.count,
            alwaysOnCount: alwaysOnCount,
            expectedActivePerWeek: expectedActive,
    ...
    /// The rarest achievement (that has been earned at least once).
    ...
    /// Shows the template picker to start adding an achievement.
    func showNewAchievement() {
        isShowingTemplatePicker = true
    }

    /// Legacy alias for tests.
    var isShowingNewAchievement: Bool {
        isShowingTemplatePicker || presentedFormMode != nil
    }
    ...
    func dismissAchievementForm() {
        presentedFormMode = nil
        refresh()
    }

    func selectTemplate(_ template: AchievementTemplate?) {
        isShowingTemplatePicker = false
        presentedFormMode = .add(template: template)
    }

    func editAchievement(_ achievement: Achievement) {
        presentedFormMode = .edit(achievementId: achievement.id)
    }

    func duplicateAchievement(_ achievement: Achievement) {
        presentedFormMode = .duplicate(achievementId: achievement.id)
    }

    /// Creates a view model for the new achievement sheet (legacy tests).
    func makeNewAchievementViewModel() -> NewAchievementViewModel {
        ...
        vm.onCancel = { [weak self] in
            self?.isShowingTemplatePicker = false
            self?.presentedFormMode = nil
        }
        return vm
    }

    func makeFormViewModel(for mode: AchievementFormMode) -> AchievementFormViewModel {
        let vm = AchievementFormViewModel(context: context, mode: mode)
        vm.onSave = { [weak self] in
            self?.dismissAchievementForm()
        }
        vm.onCancel = { [weak self] in
            self?.presentedFormMode = nil
        }
