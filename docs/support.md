# Support – League Keeper

Get help with League Keeper — a weekly league scorekeeper for game nights, tabletop groups, and optional Budget Commander (MTG) presets.

---

## How to get support

- **Bugs and feature requests:** [GitHub Issues](https://github.com/jacobrozell/League-Keeper-iOS/issues)

---

## App Store Support URL

When submitting to App Store Connect, set the **Support URL** to:

`https://jacobrozell.github.io/League-Keeper-iOS/support.html`

Privacy Policy URL:

`https://jacobrozell.github.io/League-Keeper-iOS/privacy.html`

---

## Common topics

- **Data:** All data stays on your device. No account or server is used.
- **Backup:** League data is stored in the app’s local container. Restore from an iOS device backup if you need to move to a new device.
- **Updates:** Install updates from the App Store to get the latest features and fixes.

---

*Replace the placeholder links and contact details above with your actual support URL and contact information before publishing.*
