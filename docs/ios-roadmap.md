...
When the app’s minimum deployment target is raised to **iOS 26**, consider adopting **Liquid Glass** (Apple’s design language from WWDC 2025) so custom UI matches system chrome.

**Adoption path:**
1. **Custom surfaces:** Apply `glassEffect()` to custom surfaces (e.g. section cards in Stats, Achievements, Tournament detail, list row backgrounds) that should visually align with the system’s translucent bars and sheets.
2. **Containers:** Use `GlassEffectContainer` only if multiple glass elements need coordinated blending (e.g. several cards on one screen).
3. **Transitions:** Use `glassEffectID()` for smooth morphing between states (e.g. list cell → detail) if we want shared-element-style transitions.

**Future (when targeting iOS 26):** Add availability-gated use of `glassEffect()` on selected custom containers. Prefer a single pattern (e.g. a custom View modifier or wrapper) so behavior is consistent. System controls (TabView, toolbars, navigation bars, sheets, alerts) will adopt Liquid Glass automatically when built with the iOS 26 SDK; this section is for custom views only.
