...
struct NewTournamentViewModelTests {
    ...
    struct CreateTournamentTests {
        ...
        @Test("Sets screen to attendance after creation when not sheet mode")
        ...
        @Test("Keeps screen on tournaments when sheet mode")
        func keepsScreenOnTournamentsWhenSheetMode() throws {
            let context = try TestHelpers.bootstrappedContext()
            TestFixtures.insertStandardPlayers(into: context)
            try context.save()
            
            let viewModel = NewTournamentViewModel(context: context)
            viewModel.isSheetMode = true
            viewModel.tournamentName = "Sheet League"
            viewModel.selectAll()
            viewModel.createTournament()
