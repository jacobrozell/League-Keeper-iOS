...
struct StatsViewModelTests {
    ...
    struct ComputedPropertiesTests {
        ...
        @Test("activeSegment defaults to standings")
        func activeSegmentDefault() throws {
            let context = try TestHelpers.bootstrappedContext()
            let viewModel = StatsViewModel(context: context)
            
            #expect(viewModel.activeSegment == .standings)
        }
        
        @Test("visibleSegments excludes weekly when no weekly standings")
        func visibleSegmentsExcludesWeeklyWhenNone() throws {
            let context = try TestHelpers.bootstrappedContext()
            TestFixtures.insertStandardPlayers(into: context)
            try context.save()
            ...
            #expect(viewModel.visibleSegments.contains(.standings))
            #expect(viewModel.visibleSegments.contains(.charts))
            #expect(viewModel.visibleSegments.contains(.players))
            #expect(viewModel.visibleSegments.contains(.weekly) == false)
        }
