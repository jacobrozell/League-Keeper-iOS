...
final class TournamentCompletionFlowTests: XCTestCase {
    ...
    func testCompleteTournamentShowsStandings() {
        ...
        // After completing final week, final standings sheet appears (from detail view)
        let finalRankings = app.staticTexts["Final Rankings"]
        let tournamentRankings = app.staticTexts["Tournament Rankings"]
        let standingsShown = finalRankings.waitForExistence(timeout: 5) || tournamentRankings.waitForExistence(timeout: 5)
    ...
    func testCloseCompletedTournament() {
        ...
        // Find and tap Close button (sheet dismisses; we stay on tournament detail)
        let closeButton = app.buttons["Close"]
        if closeButton.waitForExistence(timeout: 3) {
            closeButton.tap()
        }
        
        // After closing sheet we're on tournament detail (completed) or tournaments list
        let tournamentsScreen = app.navigationBars["Tournaments"]
        let tournamentDetail = app.navigationBars["Quick Tournament"]
        let onListOrDetail = tournamentsScreen.waitForExistence(timeout: 3) || tournamentDetail.waitForExistence(timeout: 3)
        XCTAssertTrue(onListOrDetail)
    }
    ...
    func testCompletedTournamentShowsInList() {
        ...
        // Close standings sheet (we stay on tournament detail)
        let closeButton = app.buttons["Close"]
        if closeButton.waitForExistence(timeout: 3) {
            closeButton.tap()
        }
        
        // We're on detail (nav title "Quick Tournament") or list with Completed section
        let completedSection = app.staticTexts["Completed"]
        let tournamentTitle = app.navigationBars["Quick Tournament"]
        let found = completedSection.waitForExistence(timeout: 3) || tournamentTitle.waitForExistence(timeout: 3)
        XCTAssertTrue(found)
    }
    ...
    func testViewCompletedTournamentStandings() {
        ...
        // Close standings sheet
        let closeButton = app.buttons["Close"]
        if closeButton.waitForExistence(timeout: 3) {
            closeButton.tap()
        }
        
        // We're on tournament detail (completed). Standings are shown inline with week selector,
        // or tap back to list then tap completed tournament to open standings sheet again.
        app.navigationBars.buttons.firstMatch.tap() // Back to list if on detail
        let tournamentCell = app.cells.firstMatch
        if tournamentCell.waitForExistence(timeout: 3) {
            tournamentCell.tap()
        }
        
        // Sheet or detail shows standings
        let standingsShown = app.staticTexts["Final Rankings"].waitForExistence(timeout: 3) ||
                             app.staticTexts["Tournament Rankings"].exists ||
                             app.staticTexts["View by week"].exists
        XCTAssertTrue(standingsShown)
    }
}
