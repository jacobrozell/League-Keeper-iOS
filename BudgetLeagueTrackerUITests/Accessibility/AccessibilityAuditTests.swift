...
final class AccessibilityAuditTests: XCTestCase {
    ...
    /// Skips system-generated buttons (empty label and empty identifier) e.g. Menu/chart internals on Achievements.
    private func verifyButtonsHaveLabels() {
        let buttons = app.buttons.allElementsBoundByIndex
        for button in buttons {
            let label = button.label
            let identifier = button.identifier
            // Skip system/internal elements that have no user-facing label (e.g. Menu items, chart segments)
            if label.isEmpty && identifier.isEmpty { continue }
            XCTAssertFalse(label.isEmpty, "Button should have accessibility label: \(identifier)")
        }
    }
    ...
    func testFormFieldsAccessibility() {
        ...
        guard nameField.waitForExistence(timeout: 8) || submitButton.waitForExistence(timeout: 8) else {
        ...
        for toggle in app.switches.allElementsBoundByIndex {
            ...
            if id.hasPrefix("toggle-") {
                XCTAssertFalse(toggle.label.isEmpty, "Toggle \(id) should have accessibility label")
            }
        }
    }
